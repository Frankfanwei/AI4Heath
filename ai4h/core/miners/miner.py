...
from ai4hr.base.protocol import AI4HProtocol
from ai4h.base.utils import validate_miner_blacklist, validate_signature
from ai4h.core.miners.analyzers.ehr_injection.analyzer import (
    EHRInjectionAnalyzer,
)


class AI4HMiner(BaseNeuron):
    """
    ...
    def forward(self, synapse: AI4HProtocol) -> AI4HProtocol:
        ...
        if synapse.analyzer == "EHR Injection":
            print(f"Executing the {synapse.analyzer} analyzer")
            output = self.analyzers["EHR Injection"].execute(synapse=synapse)
        else:
            bt.logging.error(
